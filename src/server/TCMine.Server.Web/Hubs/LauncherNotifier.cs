...
public sealed class LauncherNotifier(IHubContext<MainHub, ILauncherClient> hub)
{
    ...
    {
        return hub.Clients.All.ModpackVersionPublished(modpackId, versionId);
    }

    public Task ServerStatusChangedAsync(Guid serverId, GameServerStatus status)
    {
        return hub.Clients.Group(MainHub.GroupFor(serverId)).ServerStatusChanged(serverId, status);
    }

    public Task PlayerCountChangedAsync(Guid serverId, int online, int max)
    {
        return hub.Clients.Group(MainHub.GroupFor(serverId)).ServerPlayerCountChanged(serverId, online, max);
    }

    public Task ConsoleLineAsync(Guid serverId, ConsoleLineDto line)
    {
        return hub.Clients.Group(MainHub.GroupFor(serverId)).ConsoleLine(serverId, line);
    }
    ...
    {
        return hub.Clients.Group(MainHub.GroupFor(serverId)).RoleChanged(serverId, role);
    }
}