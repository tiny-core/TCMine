    ...
    IHostEnvironment env)
{
        ...
        Guid instanceId, IProgress<string>? progress = null, CancellationToken ct = default)
    {
        ...
        progress?.Report(
            $"Provisionando \"{instance.Name}\" — {ModLoaders.DisplayName(modpack.Loader)} {modpack.LoaderVersion} · Minecraft {modpack.Minecraft}");
    ...
    private int LinkServerMods(ModpackEntity modpack, string instanceDir, IProgress<string>? progress)
    {
        ...
        for (var i = 0; i < serverMods.Count; i++)
        {
            ...
            // Atualiza o progresso a cada 25 mods (e no último). O " — " faz o log coalescer numa linha só.
