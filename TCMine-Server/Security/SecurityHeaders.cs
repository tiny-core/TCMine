...
///
/// - <c>style-src 'unsafe-inline'</c>: o <c>&lt;style&gt;</c> dos design tokens no <c>App.razor</c> e os
///   <c>style=""</c> inline que o MudBlazor gera (posicionamento de popovers etc.) não têm nonce viável.
/// - <c>script-src 'self'</c> + <c>'wasm-unsafe-eval'</c>/<c>blob:</c>: os scripts do app são externos
...
///
/// Aplicado cedo no pipeline (antes dos ficheiros estáticos) para cobrir também esses. É inofensivo
    ...
    public static IApplicationBuilder UseSecurityHeaders(this IApplicationBuilder app)
    {
        return app.Use((ctx, next) =>
        {
            ...
            ctx.Response.OnStarting(() =>
            {
                ...
                headers["Content-Security-Policy"] = ContentSecurityPolicy;      // substitui (dedup)
